...
import (
	...
	"github.com/qiguanzhu/infrastructure/apogo/parse"
	"os"
	"path"
	"strconv"
	"strings"
...
func NewClient(conf *Conf) *Client {
	...
	client.Use(
		SetConfig(conf),
		SetCache(newNamespaceCahce()),
		SetReleaseKeyRepo(newCache()),
		SetHClient(NewHclient(queryTimeout)),
		SetPuller(newLongPuller(conf, longPollInterval, client.handleNamespaceUpdate)),
...
func SetPuller(p puller) settings {
	return func(client *Client) {
		client.longPuller = p
	}
}

func SetContext(parent context.Context) settings {
	return func(client *Client) {
		client.ctx, client.cancel = context.WithCancel(parent)
	}
}

func SetReleaseKeyRepo(repo *cache) settings {
	return func(client *Client) {
		client.releaseKeyRepo = repo
	}
}

func SetCache(cache *namespaceCache) settings {
	return func(client *Client) {
		client.caches = cache
	}
}

func SetConfig(conf *Conf) settings {
	return func(client *Client) {
		client.conf = conf
	}
}

func SetHClient(hc xhttp.HttpClientWrapper) settings {
	return func(client *Client) {
		client.hclient = hc
	}
}
...
func (c *Client) preload() error {
	...
	for _, v := range c.conf.NameSpaceNames {
		if _, e := c.sync(v); e != nil {
			defaultLogger.Printf("module:apogo method:preload namespace:%v, err:%v", v, err)
...
func (c *Client) SubscribeToNamespaces(namespaces ...string) error {
	return c.longPuller.addNamespaces(namespaces...)
}
...
func (c *Client) GetIntSliceWithNamespace(namespace, key string) ([]int, bool) {
	...
	for idx, intIfVal := range intSliceIfVal {
		...
		if !ok {
			defaultLogger.Printf("module:apogo method:GetStringSliceWithNamespace assertion failed")
			return []int{}, false
		}
...
func (c *Client) sync(namesapce string) (*ChangeEvent, error) {
	...
	bts, err := Request(c.hclient, url)
	defaultLogger.Printf("module:apogo method:Client.sync url:%s data:%s err:%v", url, bts, err)
